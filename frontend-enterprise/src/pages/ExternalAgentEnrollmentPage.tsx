...
import { AlertTriangle, ArrowLeft, CheckCircle2, Clipboard, Cloud, FileJson, KeyRound, LoaderCircle, RefreshCw, ShieldCheck, Wifi } from 'lucide-react';
    ...
    <main className="external-agent-enrollment-page">
      ...
      {!enrollment ? <section className="external-agent-enrollment-card">
        ...
        <section className="external-agent-enrollment-card">
          ...
          {status?.status === 'registered' && manifest?.status === 'approved' ? (draft ? <EmployeeDraftReview draft={draft} onChange={setDraft} busy={draftBusy} connectionTest={connectionTest} onSave={saveDraft} onConfirm={confirmDraft} onStartTest={startConnectionTest} /> : <div className="external-agent-loading"><LoaderCircle className="is-spinning" />正在根据已确认能力生成员工草稿…</div>) : status?.status === 'registered' && manifest ? <ManifestReview manifest={manifest} selectedAssetIds={selectedAssetIds} onSelectionChange={setSelectedAssetIds} reviewing={reviewing} onReview={reviewManifest} /> : status?.status === 'registered' ? <div className="external-agent-manifest-wait"><div className="external-agent-success"><CheckCircle2 /><div><strong>Agent 登记成功</strong><span>连接 ID：{status.connectionId}</span><small>正在等待 Agent 在本地展示扫描范围并确认上报。</small></div></div><div className="external-agent-waiting"><LoaderCircle className="is-spinning" /><span>等待能力清单…</span></div><label className="external-agent-manifest-upload"><FileJson /><span><strong>手动上传 Manifest</strong><small>适用于暂不支持接入 Skill 的 Agent；仍会执行相同校验。</small></span><input type="file" accept="application/json,.json" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadManifestFile(file); event.target.value = ''; }} /></label></div> : <>
            <div className={`external-agent-pairing-code ${expired ? 'is-expired' : ''}`}><small>{expired ? '配对码已过期' : '一次性配对码'}</small><strong>{enrollment.pairingCode}</strong><button type="button" disabled={expired} onClick={() => void copy(enrollment.pairingCode, '配对码已复制')}><Clipboard />复制</button></div>
            <ol className="external-agent-instructions"><li>在本地或云端 Agent 中安装/打开“开工吧接入助手”。</li><li>先由 Agent 展示准备扫描的目录、Skill、Tool 和工作流范围。</li><li>确认范围后输入配对码；Agent 将获得独立凭据，不需要开放本机公网端口。</li></ol>
        ...
        <aside className="external-agent-enrollment-card"><div className="external-agent-card-heading"><Cloud /><div><h2>接入边界</h2><p>适用于 Codex、Coze、自建 Agent 等</p></div></div><dl><div><dt>运行位置</dt><dd>Agent 原环境</dd></div><div><dt>平台保存</dt><dd>身份、能力元数据、状态与结果</dd></div><div><dt>默认不保存</dt><dd>源码、密钥、知识正文与思维链</dd></div><div><dt>当前协议</dt><dd>1.0 / Manifest 1.0</dd></div></dl><button type="button" className="external-agent-copy-instruction" onClick={() => void copy(`${enrollment.installInstruction}\n配对码：${enrollment.pairingCode}`, '接入指令已复制')}><Clipboard />复制完整接入指令</button></aside>
      </div>}
    </main>
  );
}

function EmployeeDraftReview({ draft, onChange, busy, connectionTest, onSave, onConfirm, onStartTest }: { draft: ImportDraftRead; onChange: (value: ImportDraftRead) => void; busy: boolean; connectionTest: ConnectionTestRead | null; onSave: () => Promise<void>; onConfirm: () => Promise<void>; onStartTest: () => Promise<void> }) {
  ...
  return <div className="external-agent-draft-review">
    ...
    {!confirmed ? <div className="external-agent-review-actions"><button type="button" disabled={busy} onClick={() => void onSave()}>保存草稿</button><Button disabled={busy || !draft.agentName.trim() || !draft.roleName.trim() || !draft.jobDescription.trim()} onClick={() => void onConfirm()}>{busy ? <LoaderCircle className="is-spinning" /> : <CheckCircle2 />}确认并创建员工</Button></div> : <ConnectionTestPanel test={connectionTest} busy={busy} onStart={onStartTest} />}
  </div>;
}
...
function ConnectionTestPanel({ test, busy, onStart }: { test: ConnectionTestRead | null; busy: boolean; onStart: () => Promise<void> }) {
