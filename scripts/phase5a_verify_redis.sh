...
docker compose -f "${compose_file}" exec -T redis redis-cli --user kaigongba-dev ping \
    | grep -qx PONG

(
    cd "${project_dir}/backend"
    KGB_REDIS_TEST_URL="redis://kaigongba-dev:${redis_password}@127.0.0.1:${redis_port}/15" \
